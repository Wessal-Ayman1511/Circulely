...
export const updateProfile = async (req, res, next) => {
  ...
  return res
    .status(200)
    .json({ success: true, message: messages.user.updatedSuccessfully });
};
...
export const uploadCoverPic = async (req, res, nex) => {
  ...
  return res
    .status(200)
    .json({ success: true, message: "Cover Pic added successfully" });
};
...
export const sendRequest = async (req, res, next) => {
  ...
  if (userFriend.friendRequsts.map(String).includes(user.id))
    return next(new Error("you already send a req to him", { cause: 401 }));
  // if friend already in friends
  if (user.friends.map(String).includes(userFriend.id))
    return next(new Error("already friends", { cause: 401 }));
  // if friend send a request to me
  if (user.friendRequsts.map(String).includes(userFriend.id))
    return next(
      new Error("this friend already in ur requests accept him", {
        cause: 401,
      }),
    );
...
export const acceptRequest = async (req, res, next) => {
  const { friendId } = req.params;
  // remove it from request and add it in the friends of both
  const promise = await Promise.all([
    User.updateOne(
      { _id: req.authUser._id },
      {
        $pull: {
          friendRequsts: friendId,
        },
        $addToSet: {
          friends: friendId,
        },
